...
} from "../dto/GithubCodeSnippetUpdateRequestDto";
...
export class GithubCodeSnippetService {
  ...
  async findAll(): Promise<GithubCodeSnippetResponseDto[]> {
    ...
    return codeSnippets.map(
      codeSnippet => this.dtoConverter.toGithubCodeSnippetResponseDto(
        codeSnippet
      ));
  }
  ...
  async create(createCodeSnippetDto: GithubCodeSnippetCreateRequestDto): Promise<GithubCodeSnippetResponseDto> {
    try {
      ...
      const created = await this.prisma.githubCodeSnippet.create({
        data: {
          ...
          githubRepository: {connect: {id: createCodeSnippetDto.githubRepositoryId}}
        }
      });
  ...
  async updateById(id: number, updateCodeSnippetDto: GithubCodeSnippetUpdateRequestDto): Promise<GithubCodeSnippetResponseDto> {
    try {
      const updateData: Partial<Prisma.GithubCodeSnippetCreateInput> = this.getUpdateData(
      ...
      if (e instanceof PrismaClientKnownRequestError) {
        if (e.code === "P2025") {
          if (e?.meta?.cause && e.meta.cause.toString().includes("GithubRepository")) {
            throw new ProjectNotFoundError();
          }
        }
      }
  ...
  private validateUpdateData(
    updateCodeSnippetDto: GithubCodeSnippetUpdateRequestDto, codeSnippet: GithubCodeSnippet
  ): void {
    const message = "End line number must be greater than or equal to start line number";
  ...
  private getUpdateData(
    ...
    if (updateCodeSnippetDto.displayName) updateData.displayName = updateCodeSnippetDto.displayName;
    if (updateCodeSnippetDto.filePath) updateData.filePath = updateCodeSnippetDto.filePath;
    if (updateCodeSnippetDto.format) updateData.format = updateCodeSnippetDto.format;
    if (updateCodeSnippetDto.startLine) updateData.startLine = updateCodeSnippetDto.startLine;
    if (updateCodeSnippetDto.endLine) updateData.endLine = updateCodeSnippetDto.endLine;
    if (updateCodeSnippetDto.description) updateData.description = updateCodeSnippetDto.description;
    if (updateCodeSnippetDto.githubRepositoryId) updateData.githubRepository = {connect: {id: updateCodeSnippetDto.githubRepositoryId}};
    return updateData;
  }
