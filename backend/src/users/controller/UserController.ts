...
export class UserController {
  ...
  async updateOwnUsername(
    @Req() request: Request,
    @Body() updateDto: UserUpdateUsernameRequestDto
  ) {
    const id: string = request["user"].id;

    const userDetails = await this.userService.updateUsernameById(
      +id,
      updateDto.username
    );
    return new DataResponseDto(userDetails);
  ...
  async updateOwnEmail(
    @Req() request: Request,
    @Body() updateDto: UserUpdateEmailRequestDto
  ) {
    const id: string = request["user"].id;

    const userDetails = await this.userService.updateEmailById(
      +id,
      updateDto.email
    );
    return new DataResponseDto(userDetails);
  ...
  async updateOwnPassword(
    @Req() request: Request,
    @Body() updateDto: UserUpdatePasswordRequestDto
  ) {
    const id: string = request["user"].id;

    const userDetails = await this.userService.updatePasswordById(
      +id,
      updateDto.password
    );
    return new DataResponseDto(userDetails);
