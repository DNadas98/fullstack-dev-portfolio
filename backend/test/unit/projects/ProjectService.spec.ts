...
describe("ProjectService", () => {
  ...
  describe("ProjectService findById", () => {
    it("should return a ProjectResponseDto object when project is found", async () => {
      ...
      mockPrismaCtx.prisma.githubRepository.findUnique.mockResolvedValue(
        mockProjects[0]
      );
      ...
      expect(
        mockPrismaCtx.prisma.githubRepository.findUnique
  ...
  describe("ProjectService create", () => {
    const createProjectDto = new ProjectCreateRequestDto(
      ...
      mockProjects[0].name,
      mockProjects[0].branchName,
      mockProjects[0].readmePath as string,
      mockProjects[0].readmeFormat,
      mockProjects[0].licensePath,
      mockProjects[0].licenseFormat,
      mockProjects[0].deploymentUrl
    );

    it("should successfully create a new project and return a ProjectResponseDto", async () => {
      mockPrismaCtx.prisma.githubRepository.create.mockResolvedValue(
        mockProjects[0]
      );
    ...
    it("should throw UniqueConstraintError when a project with the same name already exists", async () => {
      mockPrismaCtx.prisma.githubRepository.create.mockRejectedValue(
    ...
    it(
      "should throw GithubUserNotFoundError when the owner with the provided id does" +
        " not exist",
      async () => {
        mockPrismaCtx.prisma.githubRepository.create.mockRejectedValue(
          new PrismaClientKnownRequestError("", {
  ...
  describe("ProjectService updateById", () => {
    ...
    it("should successfully update a project and return the updated ProjectResponseDto", async () => {
      ...
      mockPrismaCtx.prisma.githubRepository.update.mockResolvedValue(
        updatedProject
      );
    ...
    it("should throw ProjectNotFoundError when trying to update a non-existing project", async () => {
      ...
      await expect(service.updateById(111, updateProjectDto)).rejects.toThrow(
        ProjectNotFoundError
      );
    });
  ...
  describe("ProjectService deleteById", () => {
    it("should successfully delete a project and return the deleted ProjectResponseDto", async () => {
      ...
      mockPrismaCtx.prisma.githubRepository.delete.mockResolvedValue(
        mockProjects[0]
      );

      await expect(service.deleteById(projectIdToDelete)).resolves.toBe(
        undefined
      );
    ...
    it("should throw ProjectNotFoundError when trying to delete a non-existing project", async () => {
      ...
      await expect(service.deleteById(projectIdToDelete)).rejects.toThrow(
        ProjectNotFoundError
      );
    });
  });
